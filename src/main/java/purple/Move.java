...
public class Move
{
	static String[] MOVE_STRINGS="""
Absorb	Grass	Special	20	100	25	hp UP damage 0.5
...
Big Money	Bug	Physical	100	100	5	BURN""".split("\n");
	...
	public static void buildMoves()
	{
		for(int i=0; i<MOVES.length; i++)
			MOVES[i]=new Move(MOVE_STRINGS[i]);
	}
	
	String name, type, atkType;
	int power, acc, pp;
	List<Effect> effects;
	public Move(String s)
	{
		String[] a=s.split("\t");
		name=a[0];
		MOVE_MAP.put(name, this);
		type=a[1];
		atkType=a[2];
		power=a[3].equals("-")?0:Integer.parseInt(a[3]);
		acc=a[4].equals("-")?100000:Integer.parseInt(a[4]);
		pp=a[5].equals("-")?100000:Integer.parseInt(a[5]);
		effects=a.length==6?new ArrayList<>():Effect.getEffects(a[6].split(" "), 0);
	}
	public Move(Move m, int i)
	{
		name=m.name;
		type=m.type;
		atkType=m.atkType;
		...
		while(i<m.effects.size())
			effects.add(m.effects.get(i++));
	}
	public Move(Move m)
	{
		this(m, 0);
		Effect e=effects.get(0);
		effects.set(0, new Effect(e.effect, e.amount+(int)(Math.random()*e.variation)-1));
	}
	public int priority()
	{
		if(effects.isEmpty())
	...
	public int critChance()
	{
		if(effects.isEmpty())
			return 1;
		Effect e=effects.get(0);
		return e.effect.equals("CRIT_CHANCE")?e.amount:1;
	}
	public boolean shouldPrintDamage()
	{
		if(power==0)
			return false;
		for(Effect e: effects)
			if(e.effect.equals("CHARGE")||e.effect.equals("IMMUNE"))
				return false;
		return true;
	}
