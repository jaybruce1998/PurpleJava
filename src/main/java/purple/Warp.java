...
public class Warp
{
	static String[] WARP_STRINGS="""
AgathasRoom,4,11,BrunosRoom,5,0
...
IndigoPlateauLobby,14,4,ViridianCity,21,16""".split("\n");
	...
	{
		for(String s: WARP_STRINGS)
		{
			String[] a=s.split(",");
			PokeMap m1=PokeMap.POKEMAPS.get(a[0]), m2=PokeMap.POKEMAPS.get(a[3]);
			int sr=Integer.parseInt(a[2]), sc=Integer.parseInt(a[1]), er=Integer.parseInt(a[5]), ec=Integer.parseInt(a[4]);
			m1.addWarp(sr, sc, new Warp(m2, er, ec));
			if(a[3].endsWith("Pokecenter"))
				FlyLocation.FLY_LOCATIONS.put(FlyLocation.normalize(a[3]), new FlyLocation(m1, sc, sr+1));
	...
	PokeMap pm;
	int row, col;
	public Warp(PokeMap pm, int row, int col)
	{
		this.pm=pm;
		this.row=row;
		this.col=col;
