...
public class Blocker extends Giver
{
	static String[] BLOCKER_STRINGS="""
ViridianCity;GIRL 17 9 RIGHT,Pokedex
...
Route23;BOULDER 6 136 DOWN,Move=Strength;BOULDER 7 136 DOWN,Move=Strength;BOULDER 9 136 DOWN,Move=Strength""".split("\n");
	static String[] BLOCKER_QUOTE_STRINGS="""
Oh goodness, I DO hope somebody will do something with that... secret sauce. I'm so tired of eating it myself!;Oh, you found someone who could bear to swallow it? Thank heavens, maybe now I won't have to do it!
...
You are nothing without the key.;!!""".split("\n");
	static String[] BADGE_QUOTE_STRINGS="""
Huh? You haven't even beaten Brock? Listen kid, this cave is dangerous, get out of here!;Whoa, you look pretty tough... go on through!
...
I sometimes hear strange sounds from deep inside.;Huh, you think you're pretty tough, don't you? Yeah, me too...""".split("\n");
	public static void buildBlockers()
	{
		...
		{
			String[] a=BLOCKER_STRINGS[i].split(";");
			PokeMap.POKEMAPS.get(a[0]).addBlocker(a[1], BLOCKER_QUOTE_STRINGS[i]);
		}
		PokeMap.POKEMAPS.get("Route10").addBlocker("FISHER 8 18 DOWN,Move=Flash", "Aw man, if only I'd get flashed today...;Whoa, look at you! I've never been flashed like that, follow me into this cave!");
		PokeMap.POKEMAPS.get("SafariZoneGate").addBlocker("SAFARI_ZONE_WORKER 3 4 RIGHT,Move=Big Money", "Listen pal, we only want big spenders here...;Whoa, you've got some BIG money! Come on in.");
		PokeMap.POKEMAPS.get("SafariZoneGate").addBlocker("SAFARI_ZONE_WORKER 4 4 LEFT,Move=Big Money", "Huh? Something smells... poor.;Hot dog, we've got a big-shot here! OH-KAY!");
		for(int i=0; i<BADGE_BLOCKER_STRINGS.length; i++)
		{
			String[] a=BADGE_BLOCKER_STRINGS[i].split(";");
			PokeMap.POKEMAPS.get(a[0]).addBlocker(a[1], BADGE_QUOTE_STRINGS[i], Integer.parseInt(a[2]));
		}
		for(String s: BOULDER_STRINGS)
		{
			String[] a=s.split(";");
		...
		PokeMap pm=PokeMap.POKEMAPS.get("Route23");
		for(int i=104; i<108; i+=2)
			for(int j=10; j<16; j++)
				pm.addBlocker("BOULDER "+j+" "+i+" DOWN,Move=Strength", "Ugh, this is too heavy!;Heh, light as a feather!", 3);
	}
	
	int numBadges;
	public Blocker(String s, String q)
	{
		super(s, q);
		numBadges=-1;
	}
	public Blocker(String s, String q, int n)
	{
		super(s, q);
		numBadges=n;
	}
	...
	{
		if(move!=null&&!p.hasMove(move)||numBadges>=0&&!p.leadersBeaten[numBadges]||item!=null&&!p.hasItem(item))
		{
