...
Nidoran♀,Poison,55,47,52,40,40,41
Nidorina,Poison,70,62,67,55,55,56
Nidoqueen,Poison/Ground,90,92,87,75,85,76
Nidoran♂,Poison,46,57,40,40,40,50
Nidorino,Poison,61,72,57,55,55,65
Nidoking,Poison/Ground,81,102,77,85,75,85
Clefairy,Normal,70,45,48,60,65,35
Clefable,Normal,95,70,73,95,90,60
Vulpix,Fire,38,41,40,50,65,65
Ninetales,Fire,73,76,75,81,100,100
Jigglypuff,Normal,115,45,20,45,25,20
Wigglytuff,Normal,140,70,45,85,50,45
...
Kadabra,Psychic,40,35,30,120,70,105
Alakazam,Psychic,55,50,45,135,95,120
Machop,Fighting,70,80,50,35,35,35
Machoke,Fighting,80,100,70,50,60,45
...
Krabby,Water,30,105,90,25,25,50
...
Voidwalker,Psychic/Normal,200,200,200,200,200,200""".split("\n");
	static Monster[] MONSTERS=new Monster[MONSTER_STRINGS.length];
	static Map<String, Monster> MONSTER_MAP=new HashMap<>();
	public static void buildMonsters()
	{
		for(int i=0; i<MONSTERS.length; i++)
			MONSTERS[i]=new Monster(MONSTER_STRINGS[i], i);
	}
	
	String name;
	String[] types;
	int hp, atk, def, spatk, spdef, spd, dexNum;
	Set<Move> learnable;
	public Monster()
	{
		name="";
	}
	public Monster(String s, int dexNum)
	{
		String[] a=s.split(",");
		name=a[0];
		types=a[1].split("/");
		hp=Integer.parseInt(a[2]);
		atk=Integer.parseInt(a[3]);
		def=Integer.parseInt(a[4]);
		spatk=Integer.parseInt(a[5]);
		spdef=Integer.parseInt(a[6]);
		spd=Integer.parseInt(a[7]);
		this.dexNum=dexNum;
		learnable=new HashSet<>();
		MONSTER_MAP.put(name, this);
	}
	public void addMoves(String s)
	{
		for(String m: s.split(","))
			learnable.add(Move.MOVE_MAP.get(m));
	}
	public String toString()
	{
		return dexNum+": "+name+" ("+String.join("/", types)+
