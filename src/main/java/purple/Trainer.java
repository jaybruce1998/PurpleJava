...
{
	static String[] TEAM_STRINGS="""
Route3;COOLTRAINER_F,16,9,LEFT,Lass,135,9 Pidgey,9 Pidgey;YOUNGSTER,10,6,RIGHT,Bug Catcher,100,10 Caterpie,10 Weedle,10 Caterpie;YOUNGSTER,14,4,DOWN,Youngster,165,11 Rattata,11 Ekans;YOUNGSTER,19,5,DOWN,Bug Catcher,90,9 Weedle,9 Kakuna,9 Caterpie,9 Metapod;COOLTRAINER_F,23,4,LEFT,Lass,150,10 Rattata,10 Nidoran♂;YOUNGSTER,24,6,RIGHT,Youngster,210,14 Spearow;YOUNGSTER,22,9,LEFT,Bug Catcher,110,11 Caterpie,11 Metapod;COOLTRAINER_F,33,10,UP,Lass,210,14 Jigglypuff
...
FightingDojo;HIKER,5,7,LEFT,Blackbelt,775,31 Machop,31 Mankey,31 Primeape;HIKER,3,6,RIGHT,Blackbelt,800,32 Machop,32 Machoke;HIKER,5,5,LEFT,Blackbelt,900,36 Primeape;HIKER,3,4,RIGHT,Blackbelt,775,31 Mankey,31 Mankey,31 Primeape;HIKER,5,3,DOWN,Blackbelt,925,37 Hitmonlee,37 Hitmonchan
...
CinnabarGym;SUPER_NERD,17,2,DOWN,Burglar,2700,36 Growlithe,36 Vulpix,36 Ninetales;SUPER_NERD,17,8,DOWN,Super Nerd,2700,36 Vulpix,36 Vulpix,36 Ninetales;SUPER_NERD,11,4,DOWN,Super Nerd,2850,34 Ponyta,34 Charmander,34 Vulpix,34 Growlithe;SUPER_NERD,11,8,DOWN,Burglar,1700,41 Ponyta;SUPER_NERD,11,14,DOWN,Super Nerd,1900,41 Rapidash;SUPER_NERD,3,14,DOWN,PokeBurglar,9595,49 Vulpix,59 Growlithe,49 Wartortle,59 Dragonite,92 Magikarp,81 Kakuna;SUPER_NERD,3,8,DOWN,Super Nerd,2987,37 Growlithe,37 Vulpix""".split("\n");
	static String[] LEADER_TEAM_STRINGS="""
PewterGym;SUPER_NERD,4,1,DOWN,Leader Brock,1386,12 Geodude,14 Onix;Bide
...
ViridianGym;GIOVANNI,2,1,DOWN,Leader Giovanni,4950,45 Rhyhorn,42 Dugtrio,44 Nidoqueen,45 Nidoking,50 Rhydon;Fissure""".split("\n");
...
VictoryRoad1F;BIKER,9,12,DOWN,Noah Buddy,6000,51 Pidgeot,52 Rhyhorn,53 Exeggcute,54 Gyarados,55 Alakazam,60 Raticate""".split("\n");
	static String[] E4_TEAM_STRINGS="""
LoreleisRoom;LORELEI,5,1,DOWN,Elite Four Lorelei,7000,60 Dewgong,61 Cloyster,62 Slowbro,63 Jynx,67 Lapras
...
RocketHideoutB1F;BLUE,3,3,DOWN,Programmer Jay,999999,190 Abra,130 Magikarp,135 Caterpie,140 Weedle,145 Ditto,165 Voidwalker""".split("\n");
	...
	public static void buildTrainers()
	{
		...
		{
			String[] a=s.split(";");
			PokeMap.POKEMAPS.get(a[0]).addTrainers(a);
		}
		for(int i=0; i<LEADER_TEAM_STRINGS.length; i++)
		{
			String[] a=LEADER_TEAM_STRINGS[i].split(";");
			PokeMap.POKEMAPS.get(a[0]).addLeader(i, a);
		}
		for(int i=0; i<GIO_RIVAL_TEAM_STRINGS.length; i++)
		{
	...
	{
		p.trainersBeaten=new boolean[trainS.length()];
		for(String s: TEAM_STRINGS)
		{
			String[] a=s.split(";");
			PokeMap pm=PokeMap.POKEMAPS.get(a[0]);
			for(int i=1; i<a.length; i++)
				if(trainS.charAt(tid)=='1')
					p.trainersBeaten[tid++]=true;
				else
					pm.addTrainer(a[i], i);
		}
		p.leadersBeaten=new boolean[leadS.length()];
		for(int i=0; i<LEADER_TEAM_STRINGS.length; i++)
			if(leadS.charAt(i)=='1')
		...
		p.gioRivalsBeaten=new boolean[gioRS.length()];
		for(int i=0; i<GIO_RIVAL_TEAM_STRINGS.length; i++)
			if(gioRS.charAt(i)=='1')
	...
	public Trainer(String s)
	{
		...
		bi=TRAINER_SPRITES.computeIfAbsent(a[0]+dir, k->{
			try {
				return OverworldGui.scale(ImageIO.read(Trainer.class.getResourceAsStream("/sprites/" + a[0] + "/" + dir + ".png")));
			} catch (Exception e) {
				e.printStackTrace();
		...
		type=a[4];
		reward=Integer.parseInt(a[5]);
		team=new Battler[a.length-6];
		for(int i=6; i<a.length; i++)
		{
			int j=a[i].indexOf(" ");
			team[i-6]=new Battler(Integer.parseInt(a[i].substring(0, j)), Monster.MONSTER_MAP.get(a[i].substring(j+1)));
		}
	}
	public void beat(Player p)
	{
		beaten=true;
	...
	static class Leader extends Trainer
	{
		Move move;
		public Leader(int id, String[] a)
		{
			super(a[1]);
			this.id=id;
			move=Move.MOVE_MAP.get(a[2]);
		}
		public void beat(Player p)
		{
			beaten=true;
	...
	static class GioRival extends Trainer
	{
		public GioRival(int id, String s)
		{
			super(s);
			this.id=id;
		}
	...
	static class E4Trainer extends Trainer
	{
		public E4Trainer(int id, String s)
		{
			super(s);
			this.id=id;
		}
